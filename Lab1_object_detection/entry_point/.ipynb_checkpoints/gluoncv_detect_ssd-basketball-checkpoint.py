...
class GroundTruthDetectionDataset(gluon.data.Dataset):
    ...
    def __init__(self, label_path, data_path, task, split='train'):
        ...
        with open(os.path.join(label_path,'SeqLabel.json')) as f: 
            ...
            file_data = "[" + file_data + "]"
            infor = json.loads(file_data)        
        infor = infor[0]['detection-annotations']
        for line in infor:
            if len(line['annotations']):
                self.image_info.append(line)
    ...
    def __getitem__(self, idx):
        ...
        image = mx.image.imread(os.path.join(self.data_path,info['frame']))

        boxes = info['annotations']

        label = []
        for box in boxes:
            label.append([box['left'], box['top'], box['left']+box['width'], box['top']+box['height'], int(box['class-id'])])
